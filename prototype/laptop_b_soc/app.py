...
st.set_page_config(
    page_title="Project System Hackers | SCADA Cyber Defense & AI SOC",
    page_icon="🛡️",
    layout="wide",
...
with st.sidebar:
    ...
    st.title("🛡️ SYSTEM HACKERS SOC")
    st.caption("SCADA Cyber Defense & AI SOC  \n*SIH 2026 | Team System Hackers*")
    st.markdown("---")
...
if is_offline:
    ...
    <div style="background: linear-gradient(90deg, #1e293b 0%, #334155 100%); border: 2px solid #64748b; border-radius: 12px; padding: 16px 24px; margin-bottom: 20px;">
        <h2 style="margin:0; color:#f1f5f9; font-size:1.4rem;">⚫ SYSTEM HACKERS SAFETY KILL-SWITCH ACTIVATED : SERVER OFFLINE</h2>
    ...
    <div class="soc-banner-safe">
        <h2 style="margin:0; color:#10b981; font-size:1.4rem;">🟢 SYSTEM HACKERS DEFCON 5 : INDUSTRIAL PROCESS & ACCESS SECURE</h2>
    ...
    <div class="soc-banner-danger">
        <h2 style="margin:0; color:#ffffff; font-size:1.4rem;">🚨 SYSTEM HACKERS THREAT ALERT: {analysis['threat_name']}</h2>
...
with intel_col:
    st.subheader("🧠 SYSTEM HACKERS Threat AI & MITRE")
    box_class = "threat-intel-danger" if analysis["is_anomaly"] else "threat-intel-box"
    st.markdown(f"""
    <div class="{box_class}">
        <div style="font-size:0.8rem; color:#94a3b8; font-weight:600;">SYSTEM HACKERS THREAT SIGNATURE</div>
...
# 4. Audit Log Table
st.markdown("---")
st.subheader("📋 SYSTEM HACKERS Incident & Forensic Audit Log")
if alerts:
